...
Value piece_value(Phase phase, PieceType pt) {
    ...
    int              v0 = (phase == MG ? 60 : 60) * pi->steps[0][MODALITY_CAPTURE].size()
           ...
           + (phase == MG ? 15 : 15)
               * std::count_if(pi->slider[0][MODALITY_CAPTURE].begin(),
                               pi->slider[0][MODALITY_CAPTURE].end(),
                               [](const std::pair<const Direction, int>& d) {
                                   return std::abs(d.first) == NORTH || std::abs(d.first) == 1;
                               })
           + (phase == MG ? 30 : 50)
               * std::count_if(pi->slider[0][MODALITY_QUIET].begin(),
                               pi->slider[0][MODALITY_QUIET].end(),
                               [](const std::pair<const Direction, int>& d) {
                                   return std::abs(d.first) == NORTH || std::abs(d.first) == 1;
                               });
...
void init(const Variant* v) {
    ...
    for (PieceSet ps = v->pieceTypes; ps;)
    {
        ...
        if (is_custom(pt))
        {
            PieceValue[MG][make_piece(WHITE, pt)] = PieceValue[MG][make_piece(BLACK, pt)] =
              piece_value(MG, pt);
            PieceValue[EG][make_piece(WHITE, pt)] = PieceValue[EG][make_piece(BLACK, pt)] =
              piece_value(EG, pt);
        }
    ...
    for (PieceType pt = PAWN; pt <= KING; ++pt)
    {
        ...
        const PieceInfo* pi = pieceMap.find(pt)->second;
        bool             isSlider =
          pi->slider[0][MODALITY_QUIET].size() || pi->slider[0][MODALITY_CAPTURE].size()
          || pi->hopper[0][MODALITY_QUIET].size() || pi->hopper[0][MODALITY_CAPTURE].size();
        bool isPawn = !isSlider && pi->steps[0][MODALITY_QUIET].size()
                   && !std::any_of(
                     pi->steps[0][MODALITY_QUIET].begin(), pi->steps[0][MODALITY_QUIET].end(),
                     [](const std::pair<const Direction, int>& d) { return d.first < SOUTH / 2; });
        bool isSlowLeaper =
          !isSlider
          && !std::any_of(
            pi->steps[0][MODALITY_QUIET].begin(), pi->steps[0][MODALITY_QUIET].end(),
            [](const std::pair<const Direction, int>& d) { return dist(d.first) > 1; });
        ...
        if (isSlider)
        {
            ...
            int           leaper =
              pi->steps[0][MODALITY_QUIET].size() + pi->steps[0][MODALITY_CAPTURE].size();
            int slider =
              pi->slider[0][MODALITY_QUIET].size() + pi->slider[0][MODALITY_CAPTURE].size()
              + pi->hopper[0][MODALITY_QUIET].size() + pi->hopper[0][MODALITY_CAPTURE].size();
            score = make_score(
              mg_value(score) * (lc * leaper + r1 * slider) / (lc * leaper + r0 * slider),
        ...
        {
            if (std::any_of(pi->steps[0][MODALITY_CAPTURE].begin(),
        ...
        if (v->extinctionValue == VALUE_MATE)
            score =
              -make_score(mg_value(score) / 8,
                          eg_value(score) / 8 / (1 + !pi->slider[0][MODALITY_CAPTURE].size()));
        ...
        for (Square s = SQ_A1; s <= SQ_MAX; ++s)
        {
            ...
            psq[pc][s] =
              score
              + (pt == PAWN   ? PBonus[std::min(r, RANK_8)][std::min(file_of(s), FILE_H)]
                 : pt == KING ? KingBonus[std::clamp(Rank(r - pawnRank + 1), RANK_1, RANK_8)]
                                         [std::min(f, FILE_D)]
                                  * (1 + v->capturesToHand)
                 : pt <= QUEEN
                   ? Bonus[pc][std::min(r, RANK_8)][std::min(f, FILE_D)] * (1 + v->blastOnCapture)
                 ...
                 : pt == COMMONER && v->extinctionValue == -VALUE_MATE
                   ...
                   ? KingBonus[std::clamp(Rank(r - pawnRank + 1), RANK_1, RANK_8)]
                              [std::min(f, FILE_D)]
                 : isSlider ? make_score(5, 5)
                                * (2 * f + std::max(std::min(r, Rank(v->maxRank - r)), RANK_1)
                                   - v->maxFile - 1)
                 : isPawn ? make_score(5, 5) * (2 * f - v->maxFile)
                          : make_score(10, 10) * (1 + isSlowLeaper)
                              * (f + std::max(std::min(r, Rank(v->maxRank - r)), RANK_1)
                                 - v->maxFile / 2));
            ...
            if (r == RANK_1 && f <= FILE_B
                && ((pt == KING && v->checkCounting) || (pt == COMMONER && v->blastOnCapture)))
