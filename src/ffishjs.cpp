...
class Board {
   ...
   public:
    ...
    std::string fen(bool showPromoted) const { return this->pos.fen(false, showPromoted); }
    ...
    void push_san_moves(std::string sanMoves) { return push_san_moves(sanMoves, NOTATION_SAN); }
   ...
   private:
    void resetStates() { this->states = StateListPtr(new std::deque<StateInfo>(1)); }
...
int validate_fen(std::string fen) { return validate_fen(fen, "chess"); }
}
...
EMSCRIPTEN_BINDINGS(ffish_js) {
    class_<Board>("Board")
      ...
      .function("variationSan",
                select_overload<std::string(std::string, Notation, bool)>(&Board::variation_san))
