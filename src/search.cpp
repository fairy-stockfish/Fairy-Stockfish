         ...
         && !(Limits.depth && mainThread && rootDepth > Limits.depth))
  {
      ...
      for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
      {
          ...
          if (rootDepth >= 4)
          {
              ...
              delta = Value(16 + int(prev) * prev / 19178) * (1 + rootPos.captures_to_hand());
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (   !ss->ttPv
        ...
        &&  eval - futility_margin(depth, improving) * (1 + pos.check_counting() + 2 * pos.must_capture() + pos.extinction_single_piece() + !pos.checking_permitted()) - (ss-1)->statScore / 256 >= beta
        &&  eval >= beta
    ...
    if (   !PvNode
        ...
        &&  eval >= beta
        &&  eval >= ss->staticEval
        &&  ss->staticEval >= beta - 15 * depth - improvement / 15 + 198 + complexity / 28 + 200 * (!pos.double_step_region(pos.side_to_move()) && (pos.piece_types() & PAWN))
        && !excludedMove
        ...
        && (ss->ply >= thisThread->nmpMinPly || us != thisThread->nmpColor))
    {
        ...
        Depth R = std::min(int(eval - beta) / 147, pos.must_capture() || pos.blast_on_capture() ? 0 : 5) + depth / 3 + 4 - (complexity > 753)
    ...
    probCutBeta = beta + (179 + 20 * !!pos.flag_region(~pos.side_to_move()) + 50 * pos.captures_to_hand()) * (1 + pos.check_counting() + pos.extinction_single_piece()) - 46 * improving;
             ...
             && ttValue < probCutBeta))
    {
        ...
        MovePicker mp(pos, ttMove, probCutBeta - ss->staticEval, depth - 3, &thisThread->gateHistory, &captureHistory);
        bool ttPv = ss->ttPv;
        ss->ttPv = false;
...
moves_loop: // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
          ...
          else
          {
              ...
              if (   !ss->inCheck
                  ...
                  && ss->staticEval + (122 + 138 * lmrDepth) * (1 + pos.check_counting()) + history / 60 <= alpha)
                  continue;

              // Prune moves with negative SEE (~3 Elo)
              if (!(pos.walling_rule() == DUCK) && !pos.see_ge(move, Value(-(25 + 10 * !!pos.flag_region(pos.side_to_move())) * lmrDepth * lmrDepth - 20 * lmrDepth)))
                  continue;
          }
      }
      ...
      if (ss->ply < thisThread->rootDepth * 2)
      {
          ...
          if (   !rootNode
              &&  depth >= 4 + 2 * (PvNode && tte->is_pv()) - 2 * (pos.count<KING>() == 1)
              &&  move == ttMove
              ...
              || (cutNode && (ss-1)->moveCount > 1)))
      {
          ...
          r -= ss->statScore / (15914 - 4434 * pos.captures_to_hand());
