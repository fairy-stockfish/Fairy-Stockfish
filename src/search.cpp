...
void Thread::search() {
         ...
         && !(Limits.depth && mainThread && rootDepth > Limits.depth))
  {
      ...
      for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
      {
          ...
          delta = Value(10 + int(prev) * prev / 15799) * (1 + rootPos.captures_to_hand());
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (   !ss->ttPv
        ...
        &&  eval - futility_margin(depth, improving) * (1 + pos.check_counting() + 2 * pos.must_capture() + pos.extinction_single_piece() + !pos.checking_permitted()) - (ss-1)->statScore / 306 >= beta
        &&  eval >= beta
    ...
    if (   !PvNode
        ...
        &&  eval >= beta
        &&  eval >= ss->staticEval
        &&  ss->staticEval >= beta - 21 * depth - improvement / 13 + 258 + 200 * (!pos.double_step_region(pos.side_to_move()) && (pos.piece_types() & PAWN))
        && !excludedMove
        ...
        && (ss->ply >= thisThread->nmpMinPly))
    {
        ...
        Depth R = std::min(int(eval - beta) / 173, pos.must_capture() || pos.blast_on_capture() ? 0 : 6) + depth / 3 + 4
    ...
    probCutBeta = beta + (168 + 20 * !!pos.flag_region(~pos.side_to_move()) + 50 * pos.captures_to_hand()) * (1 + pos.check_counting() + pos.extinction_single_piece()) - 61 * improving;
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
              ...
              || givesCheck)
          {
              ...
              if (!pos.see_ge(move, occupied, Value(-205 - 120 * pos.captures_to_hand()) * depth))
              {
                 if (depth < 2 - capture)
                    continue;
                 // Don't prune the move if opponent major piece is under discovered attack after the exchanges
                 // Don't prune the move if opponent King is under discovered attack after or during the exchanges
                 Bitboard leftEnemies = (pos.pieces(~us, KING) | pos.major_pieces(~us)) & occupied;
                 Bitboard attacks = Bitboard(0);
                 occupied |= to_sq(move);
                 while (leftEnemies && !attacks)
                 {
                      Square sq = pop_lsb(leftEnemies);
                      attacks |= pos.attackers_to(sq, occupied, us) & pos.pieces(us) & occupied;
                      // don't consider pieces which were already threatened/hanging before SEE exchanges
                      if (attacks && (!(pos.pieces(~us, KING) & sq) && (pos.attackers_to(sq, pos.pieces(), us) & pos.pieces(us))))
                         attacks = 0;
                 }
                 if (!attacks)
                    continue;
          ...
          else
          {
              ...
              if (   !ss->inCheck
                  ...
                  && ss->staticEval + (112 + 138 * lmrDepth) * (1 + pos.check_counting()) <= alpha)
                  continue;

              lmrDepth = std::max(lmrDepth, 0);

              // Prune moves with negative SEE (~4 Elo)
              if (!(pos.walling_rule() == DUCK) && !pos.see_ge(move, Value(-(27 + 10 * !!pos.flag_region(pos.side_to_move())) * lmrDepth * lmrDepth - 16 * lmrDepth)))
                  continue;
          }
      }
      ...
      if (ss->ply < thisThread->rootDepth * 2)
      {
          ...
          if (   !rootNode
              &&  depth >= 4 - (thisThread->completedDepth > 22) + 2 * (PvNode && tte->is_pv()) - 2 * (pos.count<KING>() == 1)
              &&  move == ttMove
          ...
          // Quiet ttMove extensions (~1 Elo)
                   ...
                   && move == ttMove
                   && move == ss->killers[0]
                   && (*contHist[0])[history_slot(movedPiece)][to_sq(move)] >= 5168)
              extension = 1;
      }
      ...
      r -= ss->statScore / (11124 + 4740 * (depth > 5 && depth < 22) - 4434 * pos.captures_to_hand());
