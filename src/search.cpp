...
void Search::Worker::start_searching() {
    ...
    if (rootPos.two_boards() && !threads.abort && CurrentProtocol == XBOARD)
    {
        ...
        while (!threads.stop && (Partner.sitRequested || (Partner.weDead && !Partner.partnerDead))
               && main_manager()->tm.elapsed(threads.nodes_searched()) < limits.time[us] - 1000)
        {}
    }
    ...
    if (CurrentProtocol == XBOARD)
    {
        ...
        if (rootPos.two_boards() && rootPos.virtual_drop(bestMove))
        {
            ...
            while (!threads.abort && !Partner.partnerDead && !Partner.fast
                   && limits.time[us] - main_manager()->tm.elapsed(threads.nodes_searched()) > Partner.opptime)
            {}
            Partner.ptell("x");
            // Find best real move
...
void Search::Worker::iterative_deepening() {
           ...
           && !(limits.depth && mainThread && rootDepth > limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV && !threads.stop; ++pvIdx)
        {
            ...
            delta     = Value(9 + avg * avg / 12480) * (1 + rootPos.captures_to_hand());
            ...
            if (completedDepth >= 8 && rootPos.two_boards() && CurrentProtocol == XBOARD)
            {
                ...
                if (limits.time[us])
                    Partner.ptell<FAIRY>("time "
                                         + std::to_string((limits.time[us] - main_manager()->tm.elapsed(threads.nodes_searched())) / 10));
                if (limits.time[~us])
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (!ss->ttPv && depth < 11 - 3 * pos.blast_on_capture()
        && eval
        ...
        && ss->ply >= thisThread->nmpMinPly && beta > VALUE_TB_LOSS_IN_MAX_PLY)
    {
        ...
        Depth R =
          std::min(int(eval - beta) / 148, pos.must_capture() || pos.blast_on_capture() ? 0 : 6)
          + depth / 3 + 4 - pos.must_capture() - !pos.checking_permitted();
    ...
    probCutBeta = beta
                + (182 + 20 * !!pos.flag_region(~pos.side_to_move()) + 50 * pos.captures_to_hand())
        ...
        while ((move = mp.next_move()) != Move::none())
            if (move != excludedMove && pos.legal(move))
            {
                ...
                ss->continuationHistory =
                  &this->continuationHistory[ss->inCheck][true][history_slot(
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != Move::none())
    {
            ...
            && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
        {
              ...
              if (capture || givesCheck)
            {
                ...
                if (!givesCheck && lmrDepth < 7 && !ss->inCheck)
                {
                    ...
                    int   futilityEval =
                      ss->staticEval + 279 + 295 * lmrDepth + PieceValue[MG][capturedPiece]
                ...
                if (!pos.see_ge(move, Value(-204 - 120 * pos.captures_to_hand()) * depth))
                    continue;
            }
            else
            {
                ...
                if (!ss->inCheck && lmrDepth < 15 && !pos.extinction_single_piece()
                    && ss->staticEval
                           + ((bestValue < ss->staticEval - 58 ? 139 : 55) + 121 * lmrDepth)
        ...
        ss->statScore = 2 * thisThread->mainHistory[us][from_to(move)]
                      ...
                      + (*contHist[0])[history_slot(movedPiece)][to_sq(move)]
                      + (*contHist[1])[history_slot(movedPiece)][to_sq(move)]
                      + (*contHist[3])[history_slot(movedPiece)][to_sq(move)] - 4409;

        // Decrease/increase reduction for moves with a good/bad history (~8 Elo)
        r -= ss->statScore / (14894 - 4434 * pos.captures_to_hand());
...
Value Search::Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        if (bestValue > VALUE_TB_LOSS_IN_MAX_PLY && pos.non_pawn_material(us))
        {
            ...
            if (!capture && (*contHist[0])[history_slot(pos.moved_piece(move))][to_sq(move)] < 0
                && (*contHist[1])[history_slot(pos.moved_piece(move))][to_sq(move)] < 0)
                continue;
...
void update_all_stats(const Position& pos,
    ...
    if (!pos.capture_stage(bestMove))
    {
        ...
        for (int i = 0; i < quietCount; ++i)
        {
            ...
            if (pos.walling())
                workerThread.gateHistory[us][gating_square(quietsSearched[i])] << -quietMoveMalus;
    ...
    else
    {
        ...
        if (pos.walling())
            workerThread.gateHistory[us][gating_square(bestMove)] << quietMoveBonus;
    }
    ...
    for (int i = 0; i < captureCount; ++i)
    {
        ...
        if (pos.walling())
            workerThread.gateHistory[us][gating_square(capturesSearched[i])] << -quietMoveMalus;
    }
}
...
std::string SearchManager::pv(const Search::Worker&     worker,
    ...
    for (size_t i = 0; i < multiPV; ++i)
    {
        ...
        if (CurrentProtocol == XBOARD)
        {
            ss << d << " " << UCI::value(v) << " " << time / 10 << " " << nodes << " "
               << rootMoves[i].selDepth << " " << nodes * 1000 / time << " " << tbHits << "\t";
        ...
        else
        {
            ...
            ss << " nodes " << nodes << " nps " << nodes * 1000 / time << " hashfull "
