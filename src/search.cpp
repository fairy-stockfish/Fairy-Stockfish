...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (   !ss->ttPv
        ...
        &&  eval - futility_margin(depth, cutNode && !ss->ttHit, improving) * (1 + pos.check_counting() + 2 * pos.must_capture() + pos.extinction_single_piece() + !pos.checking_permitted()) - (ss-1)->statScore / 321 >= beta
        &&  eval >= beta
    ...
    if (   !PvNode
        ...
        &&  eval >= beta
        &&  eval >= ss->staticEval
        &&  ss->staticEval >= beta - 24 * depth + 281 + 200 * (!pos.double_step_region(pos.side_to_move()) && (pos.piece_types() & PAWN))
        && !excludedMove
        ...
        &&  beta > VALUE_TB_LOSS_IN_MAX_PLY)
    {
        ...
        Depth R = std::min(int(eval - beta) / 152, pos.must_capture() || pos.blast_on_capture() ? 0 : 6) + depth / 3 + 4
    ...
    probCutBeta = beta + (168 + 20 * !!pos.flag_region(~pos.side_to_move()) + 50 * pos.captures_to_hand()) * (1 + pos.check_counting() + pos.extinction_single_piece()) - 70 * improving;
...
moves_loop: // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
              ...
              || givesCheck)
          {
              ...
              if (   !givesCheck
                  && lmrDepth < 7
                  && !ss->inCheck
                  && ss->staticEval + 188 + 206 * lmrDepth + PieceValue[MG][pos.piece_on(to_sq(move))]
          ...
          else
          {
              ...
              if (   !ss->inCheck
                  ...
                  && ss->staticEval + (115 + 122 * lmrDepth) * (1 + pos.check_counting()) <= alpha)
                  continue;

              lmrDepth = std::max(lmrDepth, 0);

              // Prune moves with negative SEE (~4 Elo)
              if (!(pos.walling_rule() == DUCK) && !pos.see_ge(move, Value(-(27 + 10 * !!pos.flag_region(pos.side_to_move())) * lmrDepth * lmrDepth)))
                  continue;
          }
      }
      ...
      if (ss->ply < thisThread->rootDepth * 2)
      {
          ...
          if (   !rootNode
              &&  depth >= 4 - (thisThread->completedDepth > 24) + 2 * (PvNode && tte->is_pv()) - 2 * (pos.count<KING>() == 1)
              &&  move == ttMove
      ...
      r -= ss->statScore / (10216 + 3855 * (depth > 5 && depth < 23) - 4434 * pos.captures_to_hand());
  ...
  Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    while ((move = mp.next_move()) != MOVE_NONE)
    {
            ...
            && pos.non_pawn_material(us))
        {
            ...
            if (   !capture
                && (*contHist[0])[history_slot(pos.moved_piece(move))][to_sq(move)] < 0
