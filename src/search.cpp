         ...
         && !(Limits.depth && mainThread && rootDepth > Limits.depth))
  {
      ...
      for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
      {
          ...
          if (rootDepth >= 4)
          {
              ...
              delta = Value(10 + int(prev) * prev / 15620) * (1 + rootPos.captures_to_hand());
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (   !ss->ttPv
        &&  depth < 8 - 3 * pos.blast_on_capture()
        &&  eval - futility_margin(depth, improving) * (1 + pos.check_counting() + 2 * pos.must_capture() + pos.extinction_single_piece() + !pos.checking_permitted()) - (ss-1)->statScore / 303 >= beta
        &&  eval >= beta
    ...
    if (   !PvNode
        ...
        &&  eval >= beta
        &&  eval >= ss->staticEval
        &&  ss->staticEval >= beta - 20 * depth - improvement / 13 + 233 + complexity / 25 + 200 * (!pos.double_step_region(pos.side_to_move()) && (pos.piece_types() & PAWN))
        && !excludedMove
        ...
        && (ss->ply >= thisThread->nmpMinPly || us != thisThread->nmpColor))
    {
        ...
        Depth R = std::min(int(eval - beta) / 168, pos.must_capture() || pos.blast_on_capture() ? 0 : 7) + depth / 3 + 4 - (complexity > 861)
    ...
    probCutBeta = beta + (191 + 20 * !!pos.flag_region(~pos.side_to_move()) + 50 * pos.captures_to_hand()) * (1 + pos.check_counting() + pos.extinction_single_piece()) - 54 * improving;
             ...
             && ttValue < probCutBeta))
    {
        ...
        while ((move = mp.next_move()) != MOVE_NONE)
            ...
            {
                assert(pos.capture(move) || type_of(move) != NORMAL);
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
              ...
              || givesCheck)
          {
              ...
              if (!pos.see_ge(move, Value(-222 - 120 * pos.captures_to_hand()) * depth))
                  continue;
          }
          else
          {
              ...
              if (   !ss->inCheck
                  ...
                  && ss->staticEval + (106 + 145 * lmrDepth) * (1 + pos.check_counting()) + history / 52 <= alpha)
                  continue;

              // Prune moves with negative SEE (~3 Elo)
              if (!(pos.walling_rule() == DUCK) && !pos.see_ge(move, Value(-(24 + 10 * !!pos.flag_region(pos.side_to_move())) * lmrDepth * lmrDepth - 15 * lmrDepth)))
                  continue;
          }
      }
      ...
      if (ss->ply < thisThread->rootDepth * 2)
      {
          ...
          if (   !rootNode
              &&  depth >= 4 - (thisThread->previousDepth > 24) + 2 * (PvNode && tte->is_pv()) - 2 * (pos.count<KING>() == 1)
              &&  move == ttMove
          ...
          // Quiet ttMove extensions (~0 Elo)
                   ...
                   && move == ttMove
                   && move == ss->killers[0]
                   && (*contHist[0])[history_slot(movedPiece)][to_sq(move)] >= 5177)
              extension = 1;
      }
              ...
              || (cutNode && (ss-1)->moveCount > 1)))
      {
          ...
          r -= ss->statScore / (13628 + 4000 * (depth > 7 && depth < 19) - 4434 * pos.captures_to_hand());
