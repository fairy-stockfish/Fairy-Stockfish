...
Value futility_margin(Depth d, bool noTtHit, bool improving, bool oppWorsening) {
    Value futilityMult = std::min(45 + d * 4, 85) - 20 * noTtHit;

    return futilityMult * d - (2789 * improving + 335 * oppWorsening) * futilityMult / 1024;
}
...
void Search::Worker::do_move(
  Position& pos, const Move move, StateInfo& st, const bool givesCheck, Stack* const ss) {
    ...
    if (ss != nullptr)
    {
        const int    pc = history_slot(movedPiece);
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, const bool cutNode) {
    ...
    if (cutNode
        && ss->staticEval
             >= beta - 13 * depth - 47 * improving + 365
                  + 200
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move()) != Move::none())
    {
            ...
            && !is_loss(bestValue))
        {
            ...
            else if (!ss->followPV || !PvNode)
            {
                ...
                Value futilityValue = ss->staticEval
                                    + (119 * lmrDepth + 90 * (ss->staticEval > alpha) + 164)
        ...
        if (capture)
            ss->statScore = 873 * int(PieceValue[MG][pos.captured_piece()]) / 128
        ...
        r -= ss->statScore * 439 / (4096 - 1500 * pos.captures_to_hand());
...
void update_all_stats(const Position& pos,
    ...
    else
    {
        ...
        captureHistory[movedPiece][to_sq(bestMove)][capturedPiece] << bonus * 1427 / 1024;
        if (pos.walling())
    ...
    for (Move move : capturesSearched)
    {
        ...
        if (!(pos.walling() && from_to(move) == from_to(bestMove)))
            captureHistory[movedPiece][to_sq(move)][capturedPiece] << -malus * 1489 / 1024;
        if (pos.walling())
...
void update_quiet_histories(
  const Position& pos, Stack* ss, Search::Worker& workerThread, Move move, int bonus) {
    ...
    if (ss->ply < LOW_PLY_HISTORY_SIZE)
        workerThread.lowPlyHistory[ss->ply][from_to(move)] << bonus * 712 / 1024;
...
Move Skill::pick_best(const RootMoves& rootMoves, usize multiPV) {
    ...
    int    delta    = std::min(topScore - minScore, int(PawnValueEg));
