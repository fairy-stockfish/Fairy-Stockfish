...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
        ...
        && ss->ply >= thisThread->nmpMinPly && !is_loss(beta))
    {
        ...
        Depth R =
          std::min(int(eval - beta) / 232, pos.must_capture() || pos.blast_on_capture() ? 0 : 6)
    ...
    probCutBeta = beta
                + (185 + 20 * !!pos.flag_region(~pos.side_to_move()) + 50 * pos.captures_to_hand())
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move()) != Move::none())
    {
              ...
              if (capture || givesCheck)
            {
                ...
                if (!givesCheck && lmrDepth < 7 && !ss->inCheck)
                {
                    Value futilityValue = ss->staticEval + 242 + 230 * lmrDepth
                                        + PieceValue[MG][capturedPiece] + 133 * captHist / 1024;
                ...
                if (!pos.see_ge(move, (-154 - 120 * pos.captures_to_hand()) * depth - seeHist))
                    continue;
            }
            else
            {
        ...
        if (capture)
            ss->statScore =
              846 * int(PieceValue[MG][pos.captured_piece()]) / 128
              + thisThread->captureHistory[movedPiece][to_sq(move)][type_of(pos.captured_piece())]
              - 4822;
        else if (ss->inCheck)
            ss->statScore = thisThread->mainHistory[us][from_to(move)]
                          + thisThread->gateHistory[us][gating_square(move)]
                          + (*contHist[0])[history_slot(movedPiece)][to_sq(move)] - 2771;
        else
            ss->statScore = 2 * thisThread->mainHistory[us][from_to(move)]
                          + thisThread->gateHistory[us][gating_square(move)] * 2
                          + (*contHist[0])[history_slot(movedPiece)][to_sq(move)]
                          + (*contHist[1])[history_slot(movedPiece)][to_sq(move)] - 3271;

        // Decrease/increase reduction for moves with a good/bad history
        r -= ss->statScore * 1582 / (16384 - 5900 * pos.captures_to_hand());
...
void update_all_stats(const Position&      pos,
    ...
    else
    {
        ...
        captureHistory[moved_piece][to_sq(bestMove)][captured] << bonus * 1187 / 1024;
        if (pos.walling())
    ...
    for (Move move : capturesSearched)
    {
        ...
        if (!(pos.walling() && from_to(move) == from_to(bestMove)))
            captureHistory[moved_piece][to_sq(move)][captured] << -malus * 1377 / 1024;
        if (pos.walling())
