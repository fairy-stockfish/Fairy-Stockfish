...
Variant* pawnback_variant() {
    ...
    v->mobilityRegion[WHITE][CUSTOM_PIECE_1] =
      (Rank2BB | Rank3BB | Rank4BB | Rank5BB | Rank6BB | Rank7BB | Rank8BB);
    v->mobilityRegion[BLACK][CUSTOM_PIECE_1] =
      (Rank7BB | Rank6BB | Rank5BB | Rank4BB | Rank3BB | Rank2BB | Rank1BB);
    v->mainPromotionPawnType[WHITE] = v->mainPromotionPawnType[BLACK] = CUSTOM_PIECE_1;
    v->promotionPawnTypes[WHITE] = v->promotionPawnTypes[BLACK] = piece_set(CUSTOM_PIECE_1);
    v->enPassantTypes[WHITE] = v->enPassantTypes[BLACK] = piece_set(CUSTOM_PIECE_1);
    v->nMoveRuleTypes[WHITE]                            = v->nMoveRuleTypes[BLACK] =
      NO_PIECE_SET;  // backwards pawn moves are reversible
    return v;
}
// Legan Chess
...
Variant* sittuyin_variant() {
    ...
    v->promotionRegion[WHITE] =
      make_bitboard(SQ_A8, SQ_B7, SQ_C6, SQ_D5, SQ_E5, SQ_F6, SQ_G7, SQ_H8);
    v->promotionRegion[BLACK] =
      make_bitboard(SQ_A1, SQ_B2, SQ_C3, SQ_D4, SQ_E4, SQ_F3, SQ_G2, SQ_H1);
...
Variant* seirawan_variant() {
    ...
    v->promotionPieceTypes[WHITE] =
      piece_set(ARCHBISHOP) | CHANCELLOR | QUEEN | ROOK | BISHOP | KNIGHT;
...
Variant* perfect_variant() {
    ...
    v->promotionPieceTypes[WHITE] =
      piece_set(AMAZON) | CHANCELLOR | ARCHBISHOP | QUEEN | ROOK | BISHOP | KNIGHT;
    v->promotionPieceTypes[BLACK] =
      piece_set(AMAZON) | CHANCELLOR | ARCHBISHOP | QUEEN | ROOK | BISHOP | KNIGHT;
...
Variant* spartan_variant() {
    ...
    v->promotionPieceTypes[BLACK] =
      piece_set(COMMONER) | DRAGON | ARCHBISHOP | CUSTOM_PIECE_2 | CUSTOM_PIECE_3;
...
Variant* flipersi_variant() {
    ...
    v->startFen =
      "8/8/8/8/8/8/8/8[PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPpppppppppppppppppppppppppppppppp] w 0 1";
    v->pieceDrops             = true;
    v->doubleStep             = false;
    v->castling               = false;
    v->immobilityIllegal      = false;
...
Variant* flipello_variant() {
    ...
    v->startFen =
      "8/8/8/3pP3/3Pp3/8/8/8[PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPpppppppppppppppppppppppppppppppppppppppppppppppppppppppppppp] w 0 1";
    v->passOnStalemate[WHITE] = true;
    v->passOnStalemate[BLACK] = true;
    return v;
}
...
Variant* omicron_variant() {
    ...
    v->maxFile          = FILE_L;
    v->startFen =
      "w**********w/*crnbqkbnrc*/*pppppppppp*/*10*/*10*/*10*/*10*/*PPPPPPPPPP*/*CRNBQKBNRC*/W**********W w KQkq - 0 1";
    v->add_piece(CUSTOM_PIECE_1, 'c', "DAW");  // Champion
    v->add_piece(CUSTOM_PIECE_2, 'w', "CF");   // Wizard
    v->castlingKingsideFile   = FILE_I;
    ...
    v->promotionRegion[BLACK] = Rank2BB | Rank1BB;
    v->promotionPieceTypes[WHITE] =
      piece_set(CUSTOM_PIECE_2) | CUSTOM_PIECE_1 | QUEEN | ROOK | BISHOP | KNIGHT;
    v->promotionPieceTypes[BLACK] =
      piece_set(CUSTOM_PIECE_2) | CUSTOM_PIECE_1 | QUEEN | ROOK | BISHOP | KNIGHT;
    v->doubleStepRegion[WHITE] = Rank3BB;
    v->doubleStepRegion[BLACK] = Rank8BB;
...
Variant* troitzky_variant() {
    ...
    v->maxFile = FILE_J;
    v->startFen =
      "****qk****/**rnbbnr**/*pppppppp*/*8*/10/10/*8*/*PPPPPPPP*/**RNBBNR**/****QK**** w - - 0 1";
    v->promotionRegion[WHITE] =
      make_bitboard(SQ_A6, SQ_B8, SQ_C9, SQ_D9, SQ_E10, SQ_F10, SQ_G9, SQ_H9, SQ_I8, SQ_J6);
    v->promotionRegion[BLACK] =
      make_bitboard(SQ_A5, SQ_B3, SQ_C2, SQ_D2, SQ_E1, SQ_F1, SQ_G2, SQ_H2, SQ_I3, SQ_J5);
    v->doubleStepRegion[WHITE] = Rank3BB;
    v->doubleStepRegion[BLACK] = Rank8BB;
    v->castling                = false;
    return v;
...
Variant* wolf_variant() {
    ...
    v->promotionPawnTypes[WHITE] = v->promotionPawnTypes[BLACK] =
      piece_set(PAWN) | piece_set(CUSTOM_PIECE_1);
...
Variant* flipello10_variant() {
    ...
    v->maxFile = FILE_J;
    v->startFen =
      "10/10/10/10/4pP4/4Pp4/10/10/10/10[PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPpppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppp] w - - 0 1";
...
Variant* supply_variant() {
    ...
    v->mobilityRegion[WHITE][ELEPHANT] =
      make_bitboard(SQ_C1, SQ_G1, SQ_A3, SQ_E3, SQ_I3, SQ_C5, SQ_G5);
    v->mobilityRegion[BLACK][ELEPHANT] =
      make_bitboard(SQ_C6, SQ_G6, SQ_A8, SQ_E8, SQ_I8, SQ_C10, SQ_G10);
    v->mobilityRegion[WHITE][SOLDIER] =
      Rank6BB | Rank7BB | Rank8BB | Rank9BB | Rank10BB
      | make_bitboard(SQ_A4, SQ_A5, SQ_C4, SQ_C5, SQ_E4, SQ_E5, SQ_G4, SQ_G5, SQ_I4, SQ_I5);
    v->mobilityRegion[BLACK][SOLDIER] =
...
Variant* Variant::conclude() {
    ...
    fastAttacks = !(pieceTypes & ~(CHESS_PIECES | COMMON_FAIRY_PIECES)) && kingType == KING
               && !restrictedMobility && !cambodianMoves && !diagonalLines;
    fastAttacks2 = !(pieceTypes & ~(SHOGI_PIECES | COMMON_STEP_PIECES)) && kingType == KING
                && !restrictedMobility && !cambodianMoves && !diagonalLines;
    ...
    if (nnueKing != NO_PIECE_TYPE)
    {
        ...
        if (((promotionPawnTypes[WHITE] | promotionPawnTypes[BLACK]) & nnueKing)
            ...
            || std::find(std::begin(promotedPieceType), std::end(promotedPieceType), nnueKing)
                 != std::end(promotedPieceType))
    ...
    if (nnueKing != NO_PIECE_TYPE)
    {
        ...
        if (std::count(fenBoard.begin(), fenBoard.end(), pieceToChar[make_piece(WHITE, nnueKing)])
              != 1
            || std::count(fenBoard.begin(), fenBoard.end(),
                          pieceToChar[make_piece(BLACK, nnueKing)])
                 != 1)
            nnueKing = NO_PIECE_TYPE;
    }
    ...
    endgameEval =
      endgameEval != EG_EVAL_CHESS
          || (endgameEval == EG_EVAL_CHESS && extinctionValue == VALUE_NONE
              && checkmateValue == -VALUE_MATE && stalemateValue == VALUE_DRAW && !materialCounting
              && !(flagRegion[WHITE] || flagRegion[BLACK]) && !mustCapture && !checkCounting
              && !makpongRule && !connectN && !blastOnCapture && !petrifyOnCaptureTypes
              && !capturesToHand && !twoBoards && !restrictedMobility && kingType == KING)
        ? endgameEval
        : NO_EG_EVAL;
        ...
        else
        {
            ...
            Variant*               v =
              !variant_template.empty()
                              ? parser.parse((new Variant(*variants.find(variant_template)->second))->init())
                              : parser.parse();
