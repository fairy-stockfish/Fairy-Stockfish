...
MovePicker::MovePicker(const Position& p, Move ttm, Depth d, const ButterflyHistory* mh, const DuckHistory* dh, const LowPlyHistory* lp,
                       const CapturePieceToHistory* cph, const PieceToHistory** ch, Move cm, const Move* killers, int pl)
           : pos(p), mainHistory(mh), duckHistory(dh), lowPlyHistory(lp), captureHistory(cph), continuationHistory(ch),
...
MovePicker::MovePicker(const Position& p, Move ttm, Depth d, const ButterflyHistory* mh, const DuckHistory* dh,
                       const CapturePieceToHistory* cph, const PieceToHistory** ch, Square rs)
           : pos(p), mainHistory(mh), duckHistory(dh), captureHistory(cph), continuationHistory(ch), ttMove(ttm), recaptureSquare(rs), depth(d) {
...
MovePicker::MovePicker(const Position& p, Move ttm, Value th, const DuckHistory* dh, const CapturePieceToHistory* cph)
           : pos(p), duckHistory(dh), captureHistory(cph), ttMove(ttm), threshold(th) {
  ...
  for (auto& m : *this)
      if constexpr (Type == CAPTURES)
          m.value =  int(PieceValue[MG][pos.piece_on(to_sq(m))]) * 6
                   + (*duckHistory)[pos.side_to_move()][gating_square(m)]
