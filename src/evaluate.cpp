...
constexpr int SafeCheck[][2] = {{},           {600, 600},  {805, 1292}, {650, 984},
                                {1071, 1886}, {730, 1128}, {600, 900}};
...
void Evaluation<T>::initialize() {
    ...
    kingAttackersCount[Them] =
      popcount(kingRing[Us] & (pe->pawn_attacks(Them) | shift<Down>(pos.pieces(Them, SHOGI_PAWN))));
...
Score Evaluation<T>::hand(PieceType pt) {
    ...
    if (pos.count_in_hand(Us, pt) > 0 && pt != KING)
    {
        ...
        Bitboard theirHalf =
          pos.board_bb()
          & ~forward_ranks_bb(Them,
                              relative_rank(Them, Rank((pos.max_rank() - 1) / 2), pos.max_rank()));
        ...
        if (pos.promoted_piece_type(pt) != NO_PIECE_TYPE && pos.drop_promoted())
            score +=
              make_score(std::max(PieceValue[MG][pos.promoted_piece_type(pt)] - PieceValue[MG][pt],
                                  VALUE_ZERO),
        ...
        if (pt == pos.drop_no_doubled())
            score -=
              make_score(50, 20) * std::max(pos.count_with_hand(Us, pt) - pos.max_file() - 1, 0);
    }
...
Score Evaluation<T>::king() const {
        ...
        case SHOGI_PAWN :
            if (pos.promoted_piece_type(pt))
            {
                otherChecks = attacks_bb(Us, pos.promoted_piece_type(pt), ksq, pos.pieces())
                            & attackedBy[Them][pt] & pos.promotion_zone(Them) & pos.board_bb();
                if (otherChecks & safe)
    ...
    kingDanger +=
      ...
      - 100 * bool(attackedBy[Us][KNIGHT] & attackedBy[Us][KING]) - 6 * mg_value(score) / 8
...
Score Evaluation<T>::threats() const {
    ...
    if (pos.must_capture())
    {
        ...
        score += make_score(200, 220)
               * popcount(attackedBy[Them][ALL_PIECES] & moves & ~pos.pieces() & ~attackedBy2[Us]);
    }
    ...
    if (pos.extinction_value() == -VALUE_MATE)
    {
        ...
        for (PieceSet ps = pos.extinction_piece_types(); ps;)
        {
            ...
            if (pos.blast_on_capture())
            {
                ...
                score +=
                  make_score(1000, 1000) / (denom * denom) * popcount(bExt & pos.pieces(Them, pt));
        }
    }
...
Score Evaluation<T>::passed() const {
    ...
    while (b)
    {
        ...
        int r = std::max(RANK_8
                           - std::max(relative_rank(Us, pos.promotion_square(Us, s), pos.max_rank())
    ...
    for (PieceSet ps = (pos.promotion_pawn_types(Us) & ~piece_set(PAWN)); ps;)
    {
        ...
        {
            Square s = pop_lsb(b2);
            if (pos.promotion_square(Us, s) == SQ_NONE
                || (pos.pieces(Them, pt) & forward_file_bb(Us, s)))
                continue;
            int r =
              std::max(RANK_8
                         - std::max(relative_rank(Us, pos.promotion_square(Us, s), pos.max_rank())
    ...
    if (pt != NO_PIECE_TYPE)
    {
        ...
        while (b)
        {
            ...
            if ((pos.pieces(Them, SHOGI_PAWN) & forward_file_bb(Us, s))
                || pos.promotion_square(Us, s) == SQ_NONE)
                continue;
...
Score Evaluation<T>::variant() const {
        ...
        for (int dist = 0; (ctfPieces || onHold || onHold2) && (ctfTargets & ~processed); dist++)
        {
            ...
            while (current)
            {
                ...
                Bitboard attacks = ((PseudoAttacks[Us][ptCtf][s] & pos.pieces())
                                    | (PseudoMoves[0][Us][ptCtf][s] & ~pos.pieces()))
    ...
    if (pos.flip_enclosed_pieces())
    {
        ...
        {
            Bitboard edges = (FileABB | file_bb(pos.max_file()) | Rank1BB | rank_bb(pos.max_rank()))
                           & pos.board_bb();
...
Value Evaluation<T>::winnable(Score score) const {
    ...
    if (pos.extinction_value() == VALUE_NONE && !pos.captures_to_hand() && !pos.connect_n()
        && !pos.material_counting() && !(pos.flag_region(WHITE) || pos.flag_region(BLACK)))
    {
        int outflanking =
        ...
        complexity = 9 * pe->passed_count()
                   + 12 * pos.count(WHITE, pos.main_promotion_pawn_type(WHITE))
                       * bool(pos.main_promotion_pawn_type(WHITE))
                   + 12 * pos.count(BLACK, pos.main_promotion_pawn_type(BLACK))
                       * bool(pos.main_promotion_pawn_type(BLACK))
                   + 15 * pos.count<SOLDIER>() + 9 * outflanking + 21 * pawnsOnBothFlanks
                   + 24 * infiltration + 51 * !pos.non_pawn_material() - 43 * almostUnwinnable
                   - 110;
    }
    ...
    if (sf == SCALE_FACTOR_NORMAL && !pos.captures_to_hand() && !pos.material_counting())
    {
        ...
        // the number of pawns of the strong side reduced if pawns are on a single flank.
            ...
            sf =
              std::min(sf, 36 + 7 * (pos.count<PAWN>(strongSide) + pos.count<SOLDIER>(strongSide)))
              - 4 * !pawnsOnBothFlanks;
...
Value Evaluation<T>::value() {
    ...
    score += king<WHITE>() - king<BLACK>() + passed<WHITE>() - passed<BLACK>() + variant<WHITE>()
           - variant<BLACK>();
...
Value fix_FRC(const Position& pos) {
    ...
    return pos.side_to_move() == WHITE ? Value(3 * correction) : -Value(3 * correction);
}
