...
Endgame<KXK, EG_EVAL_ATOMIC>  EvaluateKXKAtomic[]  = {Endgame<KXK, EG_EVAL_ATOMIC>(WHITE),
                                                      Endgame<KXK, EG_EVAL_ATOMIC>(BLACK)};
Endgame<KXK, EG_EVAL_DUCK>    EvaluateKXKDuck[]    = {Endgame<KXK, EG_EVAL_DUCK>(WHITE),
                                                      Endgame<KXK, EG_EVAL_DUCK>(BLACK)};
Endgame<KXKX, EG_EVAL_MISERE> EvaluateKXKXMisere[] = {Endgame<KXKX, EG_EVAL_MISERE>(WHITE),
                                                      Endgame<KXKX, EG_EVAL_MISERE>(BLACK)};
...
bool is_KFsPsK(const Position& pos, Color us) {
    return (pos.promotion_piece_types(us) == piece_set(FERS)) && !more_than_one(pos.pieces(~us))
...
Entry* probe(const Position& pos) {
    ...
    if (pos.captures_to_hand() || pos.two_boards())
    {
        ...
        e->materialDensity =
          (npm + npm2 + pos.count<PAWN>() * PawnValueMg) * countAll / (pos.files() * pos.ranks());
    }
    else
        e->gamePhase =
    ...
    case EG_EVAL_CHESS :
        ...
        if (!pos.count<PAWN>(WHITE) && npm_w - npm_b <= BishopValueMg)
            e->factor[WHITE] =
              uint8_t(npm_w < RookValueMg && pos.count<ALL_PIECES>(WHITE) <= 2 ? SCALE_FACTOR_DRAW
                      : npm_b <= BishopValueMg && pos.count<ALL_PIECES>(WHITE) <= 3 ? 4
                                                                                    : 14);

        if (!pos.count<PAWN>(BLACK) && npm_b - npm_w <= BishopValueMg)
            e->factor[BLACK] =
              uint8_t(npm_b < RookValueMg && pos.count<ALL_PIECES>(BLACK) <= 2 ? SCALE_FACTOR_DRAW
