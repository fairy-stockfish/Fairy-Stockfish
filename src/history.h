...
using ButterflyHistory =
  Stats<std::int16_t, 7183, COLOR_NB, int(SQUARE_NB + 1) * int(1 << SQUARE_BITS)>;

// GateHistory records the success of gating moves by the gating square (Fairy-Stockfish)
using GateHistory = Stats<std::int16_t, 7183, COLOR_NB, SQUARE_NB>;

// LowPlyHistory is addressed by ply and move's from and to squares, used
// to improve move ordering near the root
using LowPlyHistory =
  Stats<std::int16_t, 7183, LOW_PLY_HISTORY_SIZE, int(SQUARE_NB + 1) * int(1 << SQUARE_BITS)>;

// CapturePieceToHistory is addressed by a move's [piece][to][captured piece type]
using CapturePieceToHistory = Stats<i16, 10692, PIECE_NB, SQUARE_NB, PIECE_TYPE_NB>;

// PieceToHistory is like ButterflyHistory but is addressed by a move's [piece][to]
// Fairy-Stockfish: piece-indexed tables use history_slot(piece) to bound their size
constexpr int PIECE_SLOTS = 8;
int           history_slot(Piece pc);

using PieceToHistory = AtomicStats<std::int16_t, 30000, 2 * PIECE_SLOTS, SQUARE_NB>;

// ContinuationHistory is the combined history of a given pair of moves, usually
// the current one given a previous one. The nested history table is based on
// PieceToHistory instead of ButterflyBoards.
// (~63 elo)
using ContinuationHistory = MultiArray<PieceToHistory, 2 * PIECE_SLOTS, SQUARE_NB>;

// PawnHistory is addressed by the pawn structure and a move's [piece][to]
using PawnHistory =
  DynStats<AtomicStats<i16, 8192, 2 * PIECE_SLOTS, SQUARE_NB>, PAWN_HISTORY_BASE_SIZE>;
...
struct CorrHistTypedef<PieceTo> {
    using type = Stats<std::int16_t, CORRECTION_HISTORY_LIMIT, 2 * PIECE_SLOTS, SQUARE_NB>;
};

template<>
struct CorrHistTypedef<Continuation> {
    using type = MultiArray<CorrHistTypedef<PieceTo>::type, 2 * PIECE_SLOTS, SQUARE_NB>;
};
