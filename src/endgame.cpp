...
Value Endgame<KPK>::operator()(const Position& pos) const {
    ...
    if (pos.promotion_zone(us) != rank_bb(relative_rank(us, RANK_8, pos.max_rank()))
        || RANK_MAX != RANK_8 || !(pos.promotion_piece_types(us) & QUEEN))
    {
...
Value Endgame<KQKR>::operator()(const Position& pos) const {
    ...
    Value result =
      QueenValueEg - RookValueEg + push_to_edge(weakKing, pos) + push_close(strongKing, weakKing);
...
Value Endgame<KCKR>::operator()(const Position& pos) const {
    ...
    Value result = Value(push_to_edge(weakKing, pos)) + push_close(strongKing, weakKing)
...
Value Endgame<KNSFKR>::operator()(const Position& pos) const {
    ...
    Value result =
      KnightValueEg + SilverValueEg + FersValueEg - RookValueEg + push_close(winnerKSq, loserKSq)
      + push_to_opposing_edge(relative_square(strongSide, loserKSq, pos.max_rank()), pos);
...
Value Endgame<KRKS>::operator()(const Position& pos) const {
    ...
    Value result =
      RookValueEg - SilverValueEg + push_to_edge(loserKSq, pos) + push_close(winnerKSq, loserKSq);
...
Value Endgame<RK, EG_EVAL_ANTI>::operator()(const Position& pos) const {
    ...
    if (dist_min == 0)
        result +=
          strongSide == pos.side_to_move() || dist_max > 1 ? -VALUE_KNOWN_WIN : VALUE_KNOWN_WIN;
    else if (dist_min == 1)
        result +=
          weakSide == pos.side_to_move() && dist_max > 1 ? -VALUE_KNOWN_WIN : VALUE_KNOWN_WIN;
    else
...
Value Endgame<KXK, EG_EVAL_ATOMIC>::operator()(const Position& pos) const {
    ...
    Value result = pos.non_pawn_material(strongSide) + pos.count<PAWN>(strongSide) * PawnValueEg
                 + push_to_edge(loserKSq, pos) + push_away(winnerKSq, loserKSq);

    // We need at least a major and a minor, or three minors to force checkmate
    if (((pos.count<QUEEN>(strongSide) || pos.count<ROOK>(strongSide))
         && pos.count<ALL_PIECES>(strongSide) >= 3)
...
template<>
Value Endgame<KNK, EG_EVAL_ATOMIC>::operator()(const Position&) const {
    return VALUE_DRAW;
}

template<>
Value Endgame<KBK, EG_EVAL_ATOMIC>::operator()(const Position&) const {
    return VALUE_DRAW;
}

template<>
Value Endgame<KRK, EG_EVAL_ATOMIC>::operator()(const Position&) const {
    return VALUE_DRAW;
}
...
Value Endgame<KQK, EG_EVAL_ATOMIC>::operator()(const Position& pos) const {
    ...
    Value result = Value(push_to_edge(loserKSq, pos)) + push_away(winnerKSq, loserKSq);
...
template<>
Value Endgame<KNNK, EG_EVAL_ATOMIC>::operator()(const Position&) const {
    return VALUE_DRAW;
}
...
Value Endgame<KXKX, EG_EVAL_MISERE>::operator()(const Position& pos) const {
    ...
    Value result =
      pos.non_pawn_material(strongSide) * int(VALUE_KNOWN_WIN)
        / int(VALUE_KNOWN_WIN + pos.non_pawn_material(strongSide))
      - pos.non_pawn_material(weakSide) + pos.count<PAWN>(weakSide) * PawnValueEg
...
Value Endgame<KPK, EG_EVAL_DUCK>::operator()(const Position& pos) const {
    ...
    Value result = PawnValueEg + 50 * relative_rank(strongSide, strongPawn, pos.max_rank())
                 + push_to_edge(weakKing, pos) + push_close(strongKing, weakKing)
                 + push_close(strongKing, strongPawn) / 2 + push_away(weakKing, strongPawn) / 2;
...
Value Endgame<KQK, EG_EVAL_RK>::operator()(const Position& pos) const {
    ...
    if (rank_of(weakKing) < rank_of(strongQueen)
        ...
        || (Rank8BB & attacks_bb<QUEEN>(strongQueen, pos.pieces())
            & ~(attacks_bb<QUEEN>(weakKing) | attacks_bb<SHOGI_KNIGHT>(weakKing))))
...
Value Endgame<KRK, EG_EVAL_RK>::operator()(const Position& pos) const {
    ...
    if (rank_of(weakKing) < rank_of(strongRook)
        || rank_of(weakKing) + (weakSide == pos.side_to_move()) < RANK_7
        || (Rank8BB & attacks_bb<ROOK>(strongRook, pos.pieces())
            & ~(attacks_bb<QUEEN>(weakKing) | attacks_bb<SHOGI_KNIGHT>(weakKing))))
        result = VALUE_KNOWN_WIN + 100 * rank_of(strongKing);
    else
        result = -VALUE_KNOWN_WIN;
...
Value Endgame<KK, EG_EVAL_RK>::operator()(const Position& pos) const {
    ...
    Value result =
      (VALUE_KNOWN_WIN + 100 * std::max(rank_of(whiteKing), rank_of(blackKing)))
