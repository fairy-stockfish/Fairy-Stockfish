...
std::ostream& operator<<(std::ostream& os, const Position& pos) {
    ...
    for (Rank r = pos.max_rank(); r >= RANK_1; --r)
    {
        for (File f = FILE_A; f <= pos.max_file(); ++f)
            ...
            else if (((pos.captures_to_hand() && !pos.drop_loop()) || pos.two_boards())
                     && pos.is_promoted(make_square(f, r)))
    ...
    if (!isdigit(ss.peek()) && !sfen)
    {
        ...
        while ((ss >> token) && !isspace(token))
        {
            ...
            if (castling_enabled() && token == 'K')
                for (rsq = make_square(var->castlingRookKingsideFile, castling_rank(c));
                     (!(castling_rook_pieces(c) & type_of(piece_on(rsq)))
                      || color_of(piece_on(rsq)) != c)
                     && file_of(rsq) > FILE_A;
                     --rsq)
                {}

            else if (castling_enabled() && token == 'Q')
                for (rsq = make_square(var->castlingRookQueensideFile, castling_rank(c));
                     (!(castling_rook_pieces(c) & type_of(piece_on(rsq)))
                      || color_of(piece_on(rsq)) != c)
                     && file_of(rsq) < max_file();
                     ++rsq)
                {}
            ...
            if (castling_enabled() && st->castlingKingSquare[c] == SQ_NONE)
            {
                Bitboard castlingKings =
                  pieces(c, castling_king_piece(c)) & rank_bb(castling_rank(c));
                ...
                st->castlingKingSquare[c] =
                  isChess960 && piece_on(rsq) == make_piece(c, castling_king_piece(c)) ? rsq
        ...
        // Ignore if square is invalid or not on side to move relative rank 6.
                   ...
                   && ((ss >> row) && (row >= '1' && row <= '1' + max_rank())))
            {
                ...
                if ((var->enPassantRegion[sideToMove] & epSquare)
                    && (!var->fastAttacks || (var->enPassantTypes[sideToMove] & ~piece_set(PAWN))
                        || (pawn_attacks_bb(~sideToMove, epSquare) & pieces(sideToMove, PAWN)
                            && ((pieces(~sideToMove, PAWN) & (epSquare + pawn_push(~sideToMove)))
                                || (pieces(~sideToMove, PAWN)
                                    & (epSquare + 2 * pawn_push(~sideToMove))))
                            && !((pieces(WHITE) | pieces(BLACK))
                                 & (epSquare | (epSquare + pawn_push(sideToMove)))))))
...
void Position::set_check_info() const {
    ...
    st->blockersForKing[WHITE] = slider_blockers(
      pieces(BLACK), count<KING>(WHITE) ? square<KING>(WHITE) : SQ_NONE, st->pinners[BLACK], BLACK);
    st->blockersForKing[BLACK] = slider_blockers(
      pieces(WHITE), count<KING>(BLACK) ? square<KING>(BLACK) : SQ_NONE, st->pinners[WHITE], WHITE);
    ...
    for (PieceSet ps = piece_types(); ps;)
    {
        ...
        st->checkSquares[pt] =
          ksq != SQ_NONE ? attacks_bb(~sideToMove, movePt, ksq, pieces()) : Bitboard(0);
    ...
    st->bikjang      = var->bikjangRule && ksq != SQ_NONE
...
Position& Position::set(const string& code, Color c, StateInfo* si) {
    ...
    string sides[] = {
      code.substr(code.find('v') != string::npos ? code.find('v') + 1 : code.find('K', 1)),  // Weak
...
string Position::fen(
  bool sfen, bool showPromoted, int countStarted, std::string holdings, Bitboard fogArea) const {
    ...
    for (Rank r = max_rank(); r >= RANK_1; --r)
    {
        ...
        {
            for (emptyCnt = 0; f <= max_file() && !(pieces() & make_square(f, r))
                               && !(fogArea & make_square(f, r));
                 ++f)
                ++emptyCnt;
            ...
            if (f <= max_file())
            {
                ...
                else
                {
                    ...
                    // Set promoted pieces
                    if (((captures_to_hand() && !drop_loop()) || two_boards() || showPromoted)
    ...
    if (chess960 && can_castle(WHITE_CASTLING)
        && popcount(pieces(WHITE, castling_king_piece(WHITE)) & rank_bb(castling_rank(WHITE))) > 1)
    ...
    if (chess960 && can_castle(BLACK_CASTLING)
        && popcount(pieces(BLACK, castling_king_piece(BLACK)) & rank_bb(castling_rank(BLACK))) > 1)
...
Bitboard Position::slider_blockers(Bitboard sliders, Square s, Bitboard& pinners, Color c) const {
    ...
    else
    {
        ...
        {
            Bitboard diags =
              diagonal_lines() & PseudoAttacks[~c][BISHOP][s] & sliders & pieces(c, ROOK);
            while (diags)
            {
...
Bitboard
Position::attackers_to(Square s, Bitboard occupied, Color c, Bitboard janggiCannons) const {
...
bool Position::legal(Move m) const {
    ...
    if (drop_opposite_colored_bishop() && type_of(m) == DROP)
    {
        if (type_of(moved_piece(m)) != BISHOP)
        {
            ...
            if (popcount((DarkSquares & to ? DarkSquares : ~DarkSquares) & pieces(us, BISHOP)) + 1
    ...
    if (immobility_illegal() && (type_of(m) == DROP || type_of(m) == NORMAL)
        && !(PseudoMoves[0][us][type_of(moved_piece(m))][to] & board_bb()))
        return false;
    ...
    if (var->extinctionPseudoRoyal)
    {
        ...
        if (capture(m) && blast_on_capture())
            occupied &=
              ~((attacks_bb<KING>(kto) & ((pieces(WHITE) | pieces(BLACK)) ^ pieces(PAWN))) | kto);
        // Petrifying a pseudo-royal piece is illegal
        if (capture(m) && (var->petrifyOnCaptureTypes & type_of(moved_piece(m)))
    ...
    if (capture(m) && (mutually_immune_types() & type_of(moved_piece(m)))
...
bool Position::pseudo_legal(const Move m) const {
    ...
    if (walling() && (!var->wallOrMove || (from == to)))
    {
        ...
        if (walling_rule() == ARROW
            && !(moves_bb(us, type_of(pc), to, pieces() ^ from) & gating_square(m)))
            return false;
        ...
        if (walling_rule() == EDGE)
        {
            ...
            if (!(validsquares & gating_square(m)))
                return false;
        };
    }
    ...
    if (type_of(pc) == PAWN)
    {
        ...
        if (!(pawn_attacks_bb(us, from) & pieces(~us) & to)         // Not a capture
            ...
            && !((from + 2 * pawn_push(us) == to)                   // Not a double push
                 && (double_step_region(us) & from) && !(pieces() & (to | (to - pawn_push(us)))))
        ...
        && !((var->petrifyOnCaptureTypes & type_of(moved_piece(m))) && capture(m)))
    {
        ...
        if (pt == JANGGI_CANNON)
        {
            if (attacks_bb(sideToMove, pt, to, occupied)
                & attacks_bb(sideToMove, pt, to, occupied & ~janggiCannons)
    ...
    default :  //CASTLING
    {
        ...
        if (castling_rank(WHITE) > RANK_1
            && ((blockers_for_king(~sideToMove) & rfrom)
                || (non_sliding_riders() & pieces(sideToMove)))
            && attackers_to(square<KING>(~sideToMove), (pieces() ^ kfrom ^ rfrom) | rto | kto,
                            sideToMove))
            return true;
...
void Position::do_move(Move m, StateInfo& newSt, bool givesCheck, DirtyPiece& dp) {
    ...
    else
    {
        ...
        if (type_of(m) == PROMOTION || (type_of(m) == PIECE_PROMOTION && !piece_demotion())
    ...
    if (flip_enclosed_pieces() && !is_pass(m))
    {
        ...
        {
            Bitboard b =
              attacks_bb(us, QUEEN, to, ~pieces(~us)) & ~PseudoAttacks[us][KING][to] & pieces(us);
            while (b)
        ...
        else
        {
            ...
            if ((flip_enclosed_pieces() == ATAXX)
                || (flip_enclosed_pieces() == QUADWRANGLE
                    && (PseudoAttacks[us][KING][to] & pieces(us) || type_of(m) == NORMAL)))
            {
        ...
        if (castling_dropped_piece() && rank_of(to) == castling_rank(us))
        {
            if (type_of(pc) == castling_king_piece(us) && file_of(to) == castling_king_file())
            {
                ...
                Bitboard castling_rooks =
                  pieces(us) & rank_bb(castling_rank(us)) & (file_bb(FILE_A) | file_bb(max_file()));
                while (castling_rooks)
                {
                    Square s = pop_lsb(castling_rooks);
            ...
            else if (castling_rook_pieces(us) & type_of(pc))
            {
                if ((file_of(to) == FILE_A || file_of(to) == max_file())
                    && piece_on(make_square(castling_king_file(), castling_rank(us)))
                         == make_piece(us, castling_king_piece(us)))
                {
    ...
    {
        Piece promotion = make_piece(
          us, type_of(m) == PROMOTION ? promotion_type(m) : promoted_piece_type(type_of(pc)));
    ...
    // Set en passant square(s) if the moved piece can be captured
             ...
             && ((PseudoMoves[1][us][type_of(pc)][from] & ~PseudoMoves[0][us][type_of(pc)][from])
                 & to))
    {
        ...
        Bitboard blast =
          blast_on_capture()
            ? ((attacks_bb<KING>(to) & ((pieces(WHITE) | pieces(BLACK)) ^ pieces(PAWN))) | to)
                & (pieces() ^ blastImmune)
          : var->petrifyOnCaptureTypes & type_of(pc) ? square_bb(to)
                                                     : Bitboard(0);
        while (blast)
        {
            ...
            if (bsq == to ? bool(var->petrifyOnCaptureTypes & type_of(bpc))
                          : var->petrifyBlastPieces)
            {
                st->wallSquares |= bsq;
    ...
    if (counting_rule())
    {
        ...
        if ((!st->countingLimit
             || ((captured || type_of(m) == PROMOTION) && count<ALL_PIECES>(sideToMove) == 1))
            && count_limit(sideToMove))
        {
            st->countingLimit = 2 * count_limit(sideToMove);
            st->countingPly = counting_rule() == ASEAN_COUNTING || count<ALL_PIECES>(sideToMove) > 1
                              ? 0
                              : 2 * count<ALL_PIECES>();
        }
    }
...
void Position::undo_move(Move m) {
    ...
    if (st->capturedPiece && (blast_on_capture() || var->petrifyOnCaptureTypes))
    {
        ...
        while (blast)
        {
            ...
            Piece  bpc =
              st->demotedBycatch & bsq
                 ? make_piece(color_of(unpromotedBpc), promoted_piece_type(type_of(unpromotedBpc)))
                 : unpromotedBpc;
...
Key Position::key_after(Move m) const {
    ...
    if (captured)
    {
        ...
        if (captures_to_hand())
        {
            Piece removeFromHand =
              !drop_loop() && is_promoted(to)
                ? make_piece(~color_of(captured), main_promotion_pawn_type(color_of(captured)))
                : ~captured;
            k ^= Zobrist::inHand[removeFromHand]
                                [pieceCountInHand[color_of(removeFromHand)][type_of(removeFromHand)]
                                 + 1]
...
Value Position::blast_see(Move m) const {
    ...
    Bitboard blast =
      ((attacks_bb<KING>(to) & ~pieces(PAWN)) | fromto) & (pieces(WHITE) | pieces(BLACK));
        ...
        while (attackers)
        {
            ...
            if (!(extinction_piece_types() & type_of(piece_on(s))))
                minAttacker = std::min(minAttacker,
                                       blast & s ? VALUE_ZERO : CapturePieceValue[MG][piece_on(s)]);
        }
...
bool Position::see_ge(Move m, Bitboard& occupied, Value threshold) const {
    ...
    if (endgame_eval() == EG_EVAL_MISERE || must_capture() || !checking_permitted() || is_gating(m)
    ...
    occupied = (type_of(m) != DROP ? pieces() ^ from : pieces())
...
bool Position::is_optional_game_end(Value& result, int ply, int countStarted) const {
        ...
        && (!checkers() || MoveList<LEGAL>(*this).size()))
    {
        ...
        if (var->chasingRule == AXF_CHASING && st->pliesFromNull >= 20)
        {
            ...
            offset = 2 * std::max(std::max(checkThem, checkUs) - 10, 0)
                   + 20 * (CurrentProtocol == UCCI || CurrentProtocol == UCI_CYCLONE);
        }
    ...
    if (n_fold_rule())
    {
        ...
        if (end >= 4)
        {
            ...
            bool perpetualUs =
              var->perpetualCheckIllegal && st->previous->checkersBB && stp->previous->checkersBB;
            Bitboard chaseThem = undo_move_board(st->chased, st->previous->move) & stp->chased;
            Bitboard chaseUs =
              undo_move_board(st->previous->chased, stp->move) & stp->previous->chased;
            int moveRepetition = var->moveRepetitionIllegal && type_of(st->move) == NORMAL
            ...
            for (int i = 4; i <= end; i += 2)
            {
                ...
                if (i != st->pliesFromNull)
                    chaseThem = undo_move_board(chaseThem, stp->previous->move)
                ...
                if (stp->key == st->key
                    && ++cnt + 1 >= (ply > i && !moveRepetition && !chaseUs && !chaseThem
                                         && !perpetualUs && !perpetualThem
                                       ? 2
                                       : n_fold_rule()))
                {
                    result = convert_mate_value(
                      (perpetualThem || perpetualUs)                   ? (!perpetualUs     ? VALUE_MATE
                                                                          : !perpetualThem ? -VALUE_MATE
                                                                                           : VALUE_DRAW)
                      : (chaseThem || chaseUs)                         ? (!chaseUs     ? VALUE_MATE
    ...
    if (sittuyin_promotion() && count<ALL_PIECES>(sideToMove) == 2 && count<PAWN>(sideToMove) == 1
        && !checkers())
    {
...
bool Position::is_immediate_game_end(Value& result, int ply) const {
    ...
    if ((!flag_move() || var->flagPieceSafe
         || flag_piece(sideToMove) == KING)  // we can do early win detection only for the king
        && flag_reached(~sideToMove))
    {
        bool gameEnd = true;
        // Check whether king can move to CTF zone (racing kings) to draw
        if (flag_move() && flag_piece(sideToMove) == KING && sideToMove == BLACK && !checkers()
            ...
            && (flag_region(sideToMove) & attacks_from(sideToMove, KING, square<KING>(sideToMove))))
        {
            ...
            for (const auto& m : MoveList<NON_EVASIONS>(*this))
                if (type_of(moved_piece(m)) == KING && (flag_region(sideToMove) & to_sq(m))
                    && legal(m))
                {
                    gameEnd = false;
                    break;
    ...
    if (var->castlingWins)
    {
        if (st->pliesFromNull > 0 && type_of(st->move) == CASTLING)
        {
            ...
            CastlingRights justCastled =
              ~sideToMove & ((from_sq(st->move) < to_sq(st->move)) ? KING_SIDE : QUEEN_SIDE);
    ...
    if ((var->connectRegion1[~sideToMove] & connectPieces)
        && (var->connectRegion2[~sideToMove] & connectPieces))
    {
        ...
        while (true)
        {
            ...
            {
                newBitboard |=
                  shift(d, current | newBitboard)
                  & connectPieces;  // the "| newBitboard" here probably saves a few loops
            }
    ...
    if (connect_nxn())
    {
        ...
        for (int i = 1; i < connect_nxn() && connectors; i++)
            connectors &=
              shift<SOUTH>(connectors) & shift<EAST>(connectors) & shift<SOUTH_EAST>(connectors);
        if (connectors)
        {
            result = convert_mate_value(-var->connectValue, ply);
            return true;
    ...
    if ((st->pliesFromNull > 0
         && ((st->bikjang && st->previous->bikjang)
             || ((st->pass && st->previous->pass) && !var->wallOrMove)))
        || (var->adjudicateFullBoard && !(~pieces() & board_bb())))
    {
        result =
          var->materialCounting ? convert_mate_value(material_counting_result(), ply) : VALUE_DRAW;
        return true;
    }
...
Bitboard Position::chased() const {
    ...
    if (st->pliesFromNull > 0)
    {
        ...
        Bitboard newPins = st->blockersForKing[sideToMove]
                         & ~st->previous->blockersForKing[sideToMove] & pieces(sideToMove);
        while (newPins)
        {
        ...
        while (newDiscoverers)
        {
            ...
            PieceType discoveryPiece = type_of(piece_on(s));
            Bitboard  discoveryAttacks =
              attacks_from(~sideToMove, discoveryPiece, s) & pieces(sideToMove);
            ...
            while (discoveryAttacks)
            {
                ...
                if (attackers_to(s2, pieces() ^ s ^ square<KING>(sideToMove), ~sideToMove)
                    & ~square_bb(s))
                    b |= s2;
            }
        }
...
bool Position::upcoming_repetition(int ply) const {
    ...
    if (end < 3 || var->nFoldValue != VALUE_DRAW || var->perpetualCheckIllegal
        || var->materialCounting || var->moveRepetitionIllegal || walling_rule() == DUCK)
        return false;
