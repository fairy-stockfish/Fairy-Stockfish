...
class TestPyffish(unittest.TestCase):
    ...
    def test_is_optional_game_end(self):
        ...
        self.assertTrue(result[0])
        self.assertEqual(result[1], sf.VALUE_DRAW)
        # D39: Chased chariot pinned by horse + mutual chase by undermining root (controversial if pinned chariot chases)
        ...
        self.assertTrue(result[0])
        self.assertEqual(result[1], sf.VALUE_DRAW)
        # Undermining root by blocking
        result = sf.is_optional_game_end("xiangqi", "4k4/9/8c/9/7P1/8p/9/7Rn/9/3K5 w - - 0 1", 2 * ['h6i6', 'i5h5', 'i6h6', 'h5i5'])
        self.assertTrue(result[0])
        self.assertEqual(result[1], -sf.VALUE_MATE)
